...
test:
	...
	go test ./plugins/ipam/contivipam -tags="${GO_BUILD_TAGS}"
...
test-race:
	...
	go test ./plugins/ipam/contivipam -race -tags="${GO_BUILD_TAGS}"
...
test-cover: get-covtools
	...
	go test -covermode=count -coverprofile=${COVER_DIR}cov_u3.out ./plugins/ipam/contivipam -tags="${GO_BUILD_TAGS}"
