...
Each pod and node has two queueus, one for rx (`tapcli-X-output`) and one for tx (`tapcli-X-tx`). The above output is with two `nginx` pods in kubernetes.
...
In the captured packet above, we can see:
