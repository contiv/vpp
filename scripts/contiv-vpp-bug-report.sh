...
LOCAL_COMMANDS["core-dump.log"]='mkdir -p /tmp/dumps && sudo tar -zcf /tmp/dumps/`hostname`-dump.tgz /run/vpp/dumps && ls -alR /tmp/dumps'
...
then
    ...
    then
        ...
        do
            ...
            rsync --remove-source-files -e "$RSYNC_SSH" "$SSH_USER@$NODE_NAME:/tmp/dumps/*" . || true </dev/null
