...
if [ $split_k8s_version -gt 10 ] ; then
  if [ "${node_os_release}" == "16.04" ] ; then
    cat 'KUBELET_EXTRA_ARGS=--node-ip="$KUBE_WORKER_IP"' > /etc/default/kubelet
  else
    cat 'KUBELET_EXTRA_ARGS=--resolv-conf=\/run\/systemd\/resolve\/resolv.conf --node-ip="$KUBE_WORKER_IP"' > /etc/default/kubelet
  fi
