...
const (
	K8S_URL_PREFIX = "/k8s/"
	CONTIV_PREFIX  = "/contiv/"

	SERVICE_TOKEN = "/var/run/secrets/kubernetes.io/serviceaccount/token"
...
func (p *proxy) k8sHandler(w http.ResponseWriter, r *http.Request) {
	...
	url := fmt.Sprintf("https://%v:%v/%v", p.k8sHost, p.k8sPort, strings.TrimPrefix(r.URL.Path, K8S_URL_PREFIX))
...
func initProxy() (s *proxy, err error) {
	...
	cert, err := ioutil.ReadFile(ROOT_CA)
	if err != nil {
	...
	t, err := ioutil.ReadFile(SERVICE_TOKEN)
	if err != nil {
...
func main() {
	...
	http.Handle("/", fs)
	http.HandleFunc(K8S_URL_PREFIX, p.k8sHandler)
	http.HandleFunc(CONTIV_PREFIX, p.contivHandler)
