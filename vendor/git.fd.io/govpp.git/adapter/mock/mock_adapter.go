...
type VppAdapter struct {
	...
	replies       []replyWithSeqNum  // FIFO queue of messages
...
func (a *VppAdapter) MockReply(replyMsg api.Message) {
	a.repliesLock.Lock()
	defer a.repliesLock.Unlock()

	a.replies = append(a.replies, replyWithSeqNum{msg: replyMsg, hasSeqNum: false})
...
func (a *VppAdapter) MockReplyWithSeqNum(replyMsg api.Message, sequenceNum uint16) {
	a.repliesLock.Lock()
	defer a.repliesLock.Unlock()

	a.replies = append(a.replies, replyWithSeqNum{msg: replyMsg, hasSeqNum: true, seqNum: sequenceNum})
	a.mode = useRepliesQueue
}
